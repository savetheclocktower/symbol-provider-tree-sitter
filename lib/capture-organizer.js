...
function iconForTag (tag) {
  console.warn('[symbol-provider-tree-sitter] iconForTag:', tag);
  switch (tag) {
    case 'function':
      return 'icon-gear';
    case 'method':
    ...
    default:
      console.warn('[symbol-provider-tree-sitter] Unmapped icon:', tag);
      return null;
  }
}
...
class Container {
  ...
  toSymbol () {
    ...
    let symbol = {
      ...
      icon: iconForTag(nameSymbol.tag) ?? this.icon,
